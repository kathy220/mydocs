# 20220213 学习完整的因子分析 已跑通
# 来源：https://www.cnblogs.com/caiyishuai/p/12421034.html
...
def main():
    df=pd.read_excel("./data/applicants.xlsx", index_col=0)
    # print(df)
    df2=df.copy()
    print("\n原始数据:\n",df2)
    del df2['ID']
    ...
    for m in range(1, 15):
        if eig['eig_value'][:m].sum() / eig['eig_value'].sum() >= 0.80:
            print("\n公因子个数:", m)
            break
